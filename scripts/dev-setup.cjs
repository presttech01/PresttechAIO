...
// commandExists removed — the dev setup now requires a running PostgreSQL and a valid DATABASE_URL in .env
...
async function main() {
  ...
  if (!connectionString) {
    console.error('No DATABASE_URL found in .env. Please install PostgreSQL locally, create the database `presttech_ops`, and set DATABASE_URL in .env (example: postgresql://user:pass@localhost:5432/presttech_ops).');
    process.exit(1);
  }

  console.log('Using DATABASE_URL from .env to connect to PostgreSQL. Waiting for database to become available...');
  await waitForDb(connectionString);
  console.log('Database is up - applying schema (npm run db:push)...');

  const push = spawnSync('npm', ['run', 'db:push'], { cwd: root, stdio: 'inherit', shell: true, env });
  if (push.status !== 0) process.exit(push.status);

  console.log('Creating default user if none exist...');
  const createUser = spawnSync('node', ['scripts/create-default-user.cjs'], { cwd: root, stdio: 'inherit', shell: true, env });
